# This code graphs statistics for all bowlers who bowl more than 2 overs/game (Overs / Inn > 2) present in the Bowlers.csv file.
...
filtered_folder_path = os.path.join(base_folder_path, 'Filtered Bowler Graphs')
...
# Filter data to include only rows where Overs/Inn > 2
data = data[data['Ov'] / data['Inns'] > 2.5]
...
def plot_graph_and_regression(x_col, y_col, xlabel, ylabel, file_suffix):
    ...
    # Save the plot in the 'Filtered Bowler Graphs' folder
    plot_path = os.path.join(filtered_folder_path, f'{file_suffix}.png')
...
plot_graph_and_regression(y_col='Wkts', x_col='Winning Bid (cr)', ylabel='Wkts', xlabel='Winning Bid (cr)', file_suffix='Filter:Winning_Bid_vs_Wkts')
...
plot_graph_and_regression(y_col='Econ', x_col='Winning Bid (cr)', ylabel='Econ', xlabel='Winning Bid (cr)', file_suffix='Filter:Winning_Bid_vs_Econ')
